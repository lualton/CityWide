...
ccd <- readRDS("/Users/lualt/OneDrive/Work/CRPE/City Wide/ccd_2010_15.Rdata")

setwd("/Users/Alton/OneDrive/Work/CRPE/City Wide/shapefiles/shp_LA") # if on Mac 
...
### - - - - -
saveRDS(schools, "/Users/Alton/OneDrive/Work/CRPE/City Wide/LA/ccd_2010_2015_LA.Rdata")
### - - - - -
...
data <- readRDS("LA_Complete.Rdata") # Provided from Alton
...
#### - - - - - CHANGE
names(data)[3] <- "name" #School Name
names(data)[2] <- "seasch" #school ID, might need to change to num
names(data)[16] <- "year"
#### - - - - - 
...
library(tidyr)
library(dplyr)
#I did this in a previous step please refer to the memphis file prep file for most updated file prep 
#procedure
...
# names(data)[4] <- "profAdv.ELA"
# names(data)[5] <- "profAdv.Math"

tapply(data$profAdv.ELA, data$year, mean, na.rm=T) # THIS IS A TEST
tapply(data$profAdv.Math, data$year, mean, na.rm=T) # THIS IS A TEST

# Should be pcific and asian
data$propasian <- rowSums(cbind(data$asian_enrollment_total, data$hawaiian_pacific_enrollment_total), 
                          na.rm=T) / data$total_enrollment
data$numasian <- rowSums(cbind(data$asian_enrollment_total, data$hawaiian_pacific_enrollment_total), na.rm=T) 
data$propfrl <- data$free_reduced_lunch_enrollment_total/data$total_enrollment
...
selectvars <- data[,c("year","seasch","name","profAdv.ELA",
                      "profAdv.Math","free_reduced_lunch_enrollment_total","propfrl",
...
##### 
saveRDS(selectvars, "LA_complete_analysis.Rdata")
##### 
